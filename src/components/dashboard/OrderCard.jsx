...
const OrderCard = ({ order, onRateProduct, showRating = false }) => {
        ...
        for (const subOrder of subOrders) {
            ...
            for (const item of items) {
                ...
                try {
                    ...
                    console.log('No rating yet for product:', productId);
                    ratings[productId] = 0;
                }
            }
        }
    ...
    // DEBUG FUNCTION
    const handleDebugPurchase = async (productId) => {
        try {
            const response = await api.get(`/products/${productId}/debug-purchase/`);
            console.log('🔍 DEBUG PURCHASE CHECK:', response);
            alert(JSON.stringify(response, null, 2));
        } catch (err) {
            console.error('Debug error:', err);
            console.error('Error response:', err.response?.data);
            alert('Debug Error: ' + JSON.stringify(err.response?.data || err.message, null, 2));
        }
    };
    ...
    subOrders.forEach(subOrder => {
        ...
        items.forEach(item => {
            allItems.push({
                ...item,
                producerName: subOrder.producer_details?.shop_name || 'Unknown Producer',
                        ...
                        <div key={item.id || index}>
                            ...
                            {canRate && (
                                <div style={{
                                    ...
                                    {/* DEBUG BUTTON */}
                                    <button 
                                        onClick={() => handleDebugPurchase(item.product_id)}
                                        style={{ 
                                            padding: '4px 8px', 
                                            fontSize: '10px', 
                                            background: '#e74c3c', 
